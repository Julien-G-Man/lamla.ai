        ...
        for api_name, api_func in apis_to_try:
            try:
                ...
                if response and response.strip():
                    ...
                    if len(questions.get("mcq_questions", [])) < num_mcq or len(questions.get("short_questions", [])) < num_short:
                        # Use template questions to fill
                        template = self._generate_template_questions(text, num_mcq, num_short)
                        # Fill MCQ
                        while len(questions["mcq_questions"]) < num_mcq and template["mcq_questions"]:
                            questions["mcq_questions"].append(template["mcq_questions"].pop(0))
                        # Fill Short
                        while len(questions["short_questions"]) < num_short and template["short_questions"]:
                            questions["short_questions"].append(template["short_questions"].pop(0))
